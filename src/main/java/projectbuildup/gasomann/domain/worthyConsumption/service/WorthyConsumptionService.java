package projectbuildup.gasomann.domain.worthyConsumption.service;
...
import projectbuildup.gasomann.domain.worthyConsumption.dto.WorthyConsumptionDto;
import projectbuildup.gasomann.domain.worthyConsumption.dto.response.WorthyConsumptionResponseDto;
import projectbuildup.gasomann.domain.worthyConsumption.entity.*;
...
import projectbuildup.gasomann.global.error.exception.CCouponNotFoundException;
import projectbuildup.gasomann.global.error.exception.CUserNotFoundException;
import projectbuildup.gasomann.global.error.exception.CWorthyConsumptionNotFoundException;
...
import static projectbuildup.gasomann.domain.worthyConsumption.entity.CheckConditionType.*;
...
// Remove coupons and coupon issuances
    ...
    public boolean checkCouponDate(Coupon coupon){
        if(coupon.getIssuableStartDate().isBefore(LocalDate.now().plusDays(1))&&coupon.getIssuableEndDate().isAfter(LocalDate.now()))
            return true;
        return false;
    }
